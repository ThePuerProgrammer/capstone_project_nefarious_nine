...
func _ready():
	...
	if(brightness_slider.value != Pomotimer._game_settings.brightness):
		brightness_slider.value = Pomotimer._game_settings.brightness
	
	if(contrast_slider.value != Pomotimer._game_settings.contrast):
		contrast_slider.value = Pomotimer._game_settings.contrast
	
	if(volume_slider.value != Pomotimer._game_settings.brightness):
		volume_slider.value = Pomotimer._game_settings.volume
################################################################################
##FADING IN & OUT
################################################################################
